<template>
    <div class="flex flex-col py-5 w-full gap-2 bg-stone-50 dark:bg-stone-900 duration-150 border-stone-300 dark:border-stone-700 border-t-[1px]">
        <div class="flex flex-col w-full gap-1">
            <div class="flex justify-between items-center gap-4">
                ...
                <div class="flex gap-6 items-center min-w-max">
                    <div @click="deleteTask(task.id)" class="flex items-center gap-2 text-red-400 duration-150 hover:text-red-500 cursor-pointer select-none">
                        ...
                        <span class="hidden md:inline text-current font-inter text-sm">Usuń</span>
                    </div>
                    <div @click="showEditTaskDialog(task)" class="flex items-center gap-2 text-stone-500 duration-150 hover:text-stone-600 dark:text-stone-400 dark:hover:text-stone-300 cursor-pointer select-none">
                        <span class="text-current">
                        ...
                        <span class="hidden md:inline text-current font-inter text-sm">Edytuj</span>
                    </div>
                    <div @click="taskDone(task.id)" class="flex items-center gap-2 text-stone-500 duration-150 hover:text-stone-600 dark:text-stone-400 dark:hover:text-stone-300 cursor-pointer select-none">
                        <span class="text-current">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="10" viewBox="0 0 14 10" fill="none">
                                <path d="M1 5L5 9L13 1" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </span>
                        <span class="hidden md:inline text-current font-inter text-sm">Ukończone</span>
                    </div>
                </div>
            </div>
