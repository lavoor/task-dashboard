<template>
	<div
		class="flex rounded-xl bg-slate-200/90 dark:bg-zinc-800 focus-within:bg-slate-200/90 dark:focus-within:bg-zinc-700 h-11 px-8 items-center gap-8 transition-colors duration-150">
		<span class="text-slate-500 dark:text-zinc-400 min-w-max">
			<svg
		...
		<input
			v-model="inputValue"
			@input="handleSearch"
			class="w-full outline-none border-none font-inter text-base placeholder:text-slate-500 text-slate-800 dark:placeholder:text-slate-400 dark:text-zinc-50"
...
<script setup lang="ts">
	...
	const handleSearch = () => {
		...
		props.searchTasks(inputValue.value);
	};
</script>
