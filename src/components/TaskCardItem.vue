<template>
	<div
		class="flex w-full lg:w-[27.375rem] flex-col justify-between gap-2 bg-slate-200/90 dark:bg-zinc-800 duration-150 transition-colors p-5 rounded-xl">
		<div class="flex flex-col w-full gap-1">
			<div class="flex justify-between items-center gap-4">
				<span
					class="font-inter font-medium text-slate-800 dark:text-zinc-50 text-ellipsis overflow-hidden whitespace-nowrap"
					>{{ task.title }}</span
				>
				<div class="flex gap-6 items-center min-w-max">
					<div
						@click="deleteTask(task.id)"
						class="w-4 h-4 p-0.5 flex justify-center items-center text-red-400 duration-150 hover:text-red-500 cursor-pointer select-none">
					...
					<div
						@click="showEditTaskDialog(task)"
						class="w-4 h-4 p-0.5 flex justify-center items-center text-slate-500 duration-150 hover:text-slate-600 dark:text-zinc-400 dark:hover:text-zinc-300 cursor-pointer select-none">
						<span class="text-current">
							<svg
								xmlns="http://www.w3.org/2000/svg"
								width="14"
			...
			<p class="text-slate-500 dark:text-zinc-400 font-inter">
				{{ task.description }}
			</p>
		</div>
		<Transition mode="out-in">
			<div
				...
				class="flex gap-3 items-center text-lime-600 dark:text-lime-400">
				<span class="text-current">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="12"
				...
				<span class="text-current font-medium font-inter">Done</span>
			</div>
			<div
				v-else-if="task.status === 'inprogress'"
				class="flex gap-3 items-center text-amber-600 dark:text-orange-300">
				<span class="text-current">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="16"
			...
			<div
				v-else
				class="flex gap-3 items-center text-slate-500 dark:text-zinc-400">
				<span class="text-current">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="12"
				...
				<span class="text-current font-medium font-inter">To do</span>
			</div>
		</Transition>
	</div>
</template>
