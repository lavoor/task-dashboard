        ...
        <div class="container py-16 flex flex-col gap-6">
            ...
            <div class="w-full flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    <OptionList :selected="tasksStyle === 'list'" :chooseOption="chooseTaskStyle"/>
                    <OptionCards :selected="tasksStyle === 'cards'" :chooseOption="chooseTaskStyle"/>
                </div>
            </div>
            <div class="flex flex-col w-full">
                ...
                <div v-if="tasksStyle === 'cards' && visibleTasks.length > 0">
                    <TransitionGroup @beforeLeave="handleBeforeLeave" name="taskCard" tag="div" class="w-full flex flex-col lg:grid lg:grid-cols-2 gap-5" appear>
                        <TaskCardItem v-for="task in visibleTasks" :key="task.id" :taskDone="taskDone" :showEditTaskDialog="showEditTaskDialog" :deleteTask="deleteTask" :task="task"/>
                    </TransitionGroup>
                </div>
                <div v-else-if="visibleTasks.length > 0">
                    <TransitionGroup name="taskList" tag="div" appear>
                        <TaskListItem v-for="task in visibleTasks" :key="task.id" :taskDone="taskDone" :showEditTaskDialog="showEditTaskDialog" :deleteTask="deleteTask" :task="task"/>
                    </TransitionGroup>
                </div>
                <div v-else class="w-full">
                    <span class="font-inter font-medium text-stone-500 dark:stone-400">Brak zadań...</span>
                </div>
            </div>
        </div>
        <Transition>
            <AddTask v-if="addTaskDialog" :createTask="createTask" :hideAddTaskDialog="hideAddTaskDialog"/>
        </Transition>
        <Transition>
            <EditTask v-if="editTaskDialog" :task="currentTaskEditing" :editTask="editTask" :hideEditTaskDialog="hideEditTaskDialog"/>
        </Transition>
    </div>
</template>
...
const filterTasks = () => {
    const searchedTasks = tasks.value.filter((task: Task) => {
        ...
        const searchValueNormalized = searchValue.value.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        const taskTitleNormalized = task.title.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        const taskDescriptionNormalized = task.description.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
